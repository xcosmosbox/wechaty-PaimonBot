...
tstest_1.test('Puppeteer smoke testing', async (t) => {
    let browser;
    let page;
    try {
        browser = await puppeteer_1.launch(PUPPETEER_LAUNCH_OPTIONS);
...
tstest_1.test('evaluate() a function that returns a Promise', async (t) => {
    try {
        const browser = await puppeteer_1.launch(PUPPETEER_LAUNCH_OPTIONS);
...
tstest_1.test('evaluate() a file and get the returns value', async (t) => {
    const EXPECTED_OBJ = {
        code: 42,
...
tstest_1.test('page.on(console)', async (t) => {
...
tstest_1.test('page.exposeFunction()', async (t) => {
    const browser = await puppeteer_1.launch(PUPPETEER_LAUNCH_OPTIONS);
    const page = await browser.newPage();
    const spy = tstest_1.sinon.spy();
...
tstest_1.test('other demos', async (t) => {
