...
class Bridge extends events_1.EventEmitter {
    constructor(options) {
        ...
        config_1.log.verbose('PuppetWeChatBridge', 'constructor()');
        this.state = new state_switch_1.StateSwitch('PuppetWeChatBridge', config_1.log);
    }
    async start() {
        config_1.log.verbose('PuppetWeChatBridge', 'start()');
        this.state.on('pending');
        try {
            this.browser = await this.initBrowser();
            config_1.log.verbose('PuppetWeChatBridge', 'start() initBrowser() done');
            ...
            await ready;
            this.state.on(true);
            config_1.log.verbose('PuppetWeChatBridge', 'start() initPage() done');
        }
        catch (e) {
            config_1.log.error('PuppetWeChatBridge', 'start() exception: %s', e);
            this.state.off(true);
            try {
                if (this.page) {
                ...
                config_1.log.error('PuppetWeChatBridge', 'start() exception %s, close page/browser exception %s', e, e2);
            }
            this.emit('error', e);
            throw e;
        }
    ...
    async initBrowser() {
        config_1.log.verbose('PuppetWeChatBridge', 'initBrowser()');
        ...
        config_1.log.verbose('PuppetWeChatBridge', 'initBrowser() with options=%s', JSON.stringify(options));
        let browser;
        if (!this.options.stealthless) {
            /**
              ...
              */
            const plugin = puppeteer_extra_plugin_stealth_1.default();
            plugin.onBrowser = () => { };
            puppeteer_extra_1.default.use(plugin);
            browser = await puppeteer_extra_1.default.launch(options);
        }
        else {
            browser = await puppeteer_1.launch(options);
        }
        const version = await browser.version();
        config_1.log.verbose('PuppetWeChatBridge', 'initBrowser() version: %s', version);
        return browser;
    }
    async onDialog(dialog) {
        config_1.log.warn('PuppetWeChatBridge', 'onDialog() page.on(dialog) type:%s message:%s', dialog.type, dialog.message());
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'onDialog() dialog.dismiss() reject: %s', e);
        }
        this.emit('error', new Error(`${dialog.type}(${dialog.message()})`));
    }
    async onLoad(page) {
        config_1.log.verbose('PuppetWeChatBridge', 'onLoad() page.url=%s', page.url());
        if (this.state.off()) {
            config_1.log.verbose('PuppetWeChatBridge', 'onLoad() OFF state detected. NOP');
        ...
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'onLoad() exception: %s', e);
            await page.close();
            this.emit('error', e);
        }
    }
    async initPage(browser) {
        config_1.log.verbose('PuppetWeChatBridge', 'initPage()');
        ...
        const cookieList = (await this.options.memory.get(config_1.MEMORY_SLOT));
        const url = this.entryUrl(cookieList);
        config_1.log.verbose('PuppetWeChatBridge', 'initPage() before page.goto(url)');
        // Does this related to(?) the CI Error: exception: Navigation Timeout Exceeded: 30000ms exceeded
        await page.goto(url);
        config_1.log.verbose('PuppetWeChatBridge', 'initPage() after page.goto(url)');
        // await this.uosPatch(page)
        void this.uosPatch;
        if (cookieList && cookieList.length) {
            await page.setCookie(...cookieList);
            config_1.log.silly('PuppetWeChatBridge', 'initPage() page.setCookie() %s cookies set back', cookieList.length);
        }
    ...
    async readyAngular(page) {
        config_1.log.verbose('PuppetWeChatBridge', 'readyAngular()');
        try {
            await page.waitForFunction("typeof window.angular !== 'undefined'");
        }
        catch (e) {
            config_1.log.verbose('PuppetWeChatBridge', 'readyAngular() exception: %s', e);
            ...
            if (blockedMessage) { // Wechat Account Blocked
                ...
                config_1.log.info('PuppetWeChatBridge', `
    ...
    async inject(page) {
        config_1.log.verbose('PuppetWeChatBridge', 'inject()');
        ...
        try {
            ...
            if (retObj && /^(2|3)/.test(retObj.code.toString())) {
                // HTTP Code 2XX & 3XX
                config_1.log.silly('PuppetWeChatBridge', 'inject() eval(Wechaty) return code[%d] message[%s]', retObj.code, retObj.message);
            }
            ...
            if (retObj && /^(2|3)/.test(retObj.code.toString())) {
                // HTTP Code 2XX & 3XX
                config_1.log.silly('PuppetWeChatBridge', 'inject() Wechaty.init() return code[%d] message[%s]', retObj.code, retObj.message);
            }
            else { // HTTP Code 4XX & 5XX
            ...
            config_1.log.silly('PuppetWeChatBridge', 'inject() ding success');
        }
        catch (e) {
            config_1.log.verbose('PuppetWeChatBridge', 'inject() exception: %s. stack: %s', e.message, e.stack);
            throw e;
        }
    }
    async logout() {
        config_1.log.verbose('PuppetWeChatBridge', 'logout()');
        try {
            return await this.proxyWechaty('logout');
        }
        catch (e) {
            config_1.log.error('PuppetWeChatBridge', 'logout() exception: %s', e.message);
            throw e;
        }
    }
    async stop() {
        config_1.log.verbose('PuppetWeChatBridge', 'stop()');
        if (!this.page) {
        ...
        try {
            await this.page.close();
            config_1.log.silly('PuppetWeChatBridge', 'stop() page.close()-ed');
        }
        catch (e) {
            config_1.log.warn('PuppetWeChatBridge', 'stop() page.close() exception: %s', e);
        }
        try {
            await this.browser.close();
            config_1.log.silly('PuppetWeChatBridge', 'stop() browser.close()-ed');
        }
        catch (e) {
            config_1.log.warn('PuppetWeChatBridge', 'stop() browser.close() exception: %s', e);
        }
        this.state.off(true);
    }
    async getUserName() {
        config_1.log.verbose('PuppetWeChatBridge', 'getUserName()');
        try {
            const userName = await this.proxyWechaty('getUserName');
            return userName;
        }
        catch (e) {
            config_1.log.error('PuppetWeChatBridge', 'getUserName() exception: %s', e.message);
            throw e;
        }
    }
    async contactAlias(contactId, alias) {
        try {
            ...
            config_1.log.verbose('PuppetWeChatBridge', 'contactRemark() exception: %s', e.message);
            // Issue #509 return false instead of throw when contact is not a friend.
            // throw e
            config_1.log.warn('PuppetWeChatBridge', 'contactRemark() does not work on contact is not a friend');
            return false;
        }
    }
    async contactList() {
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'contactList() exception: %s', e.message);
            throw e;
        }
    }
    async roomList() {
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'roomList() exception: %s', e.message);
            throw e;
        }
    }
    async roomDelMember(roomId, contactId) {
        ...
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'roomDelMember(%s, %s) exception: %s', roomId, contactId, e.message);
            throw e;
        }
    }
    async roomAddMember(roomId, contactId) {
        config_1.log.verbose('PuppetWeChatBridge', 'roomAddMember(%s, %s)', roomId, contactId);
        if (!roomId || !contactId) {
            throw new Error('no roomId or contactId');
        }
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'roomAddMember(%s, %s) exception: %s', roomId, contactId, e.message);
            throw e;
        }
    }
    async roomModTopic(roomId, topic) {
        ...
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'roomModTopic(%s, %s) exception: %s', roomId, topic, e.message);
            throw e;
        }
    }
    async roomCreate(contactIdList, topic) {
        ...
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'roomCreate(%s) exception: %s', contactIdList, e.message);
            throw e;
        }
    }
    async verifyUserRequest(contactId, hello) {
        config_1.log.verbose('PuppetWeChatBridge', 'verifyUserRequest(%s, %s)', contactId, hello);
        if (!contactId) {
        ...
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'verifyUserRequest(%s, %s) exception: %s', contactId, hello, e.message);
            throw e;
        }
    }
    async verifyUserOk(contactId, ticket) {
        config_1.log.verbose('PuppetWeChatBridge', 'verifyUserOk(%s, %s)', contactId, ticket);
        if (!contactId || !ticket) {
        ...
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'verifyUserOk(%s, %s) exception: %s', contactId, ticket, e.message);
            throw e;
        }
    }
    async send(toUserName, text) {
        config_1.log.verbose('PuppetWeChatBridge', 'send(%s, %s)', toUserName, text);
        if (!toUserName) {
        ...
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'send() exception: %s', e.message);
            throw e;
        }
    }
    async getMsgImg(id) {
        config_1.log.verbose('PuppetWeChatBridge', 'getMsgImg(%s)', id);
        try {
            return await this.proxyWechaty('getMsgImg', id);
        }
        catch (e) {
            config_1.log.silly('PuppetWeChatBridge', 'proxyWechaty(getMsgImg, %d) exception: %s', id, e.message);
            throw e;
        }
    }
    async getMsgEmoticon(id) {
        config_1.log.verbose('PuppetWeChatBridge', 'getMsgEmoticon(%s)', id);
        try {
            return await this.proxyWechaty('getMsgEmoticon', id);
        }
        catch (e) {
            config_1.log.silly('PuppetWeChatBridge', 'proxyWechaty(getMsgEmoticon, %d) exception: %s', id, e.message);
            throw e;
        }
    }
    async getMsgVideo(id) {
        config_1.log.verbose('PuppetWeChatBridge', 'getMsgVideo(%s)', id);
        try {
            return await this.proxyWechaty('getMsgVideo', id);
        }
        catch (e) {
            config_1.log.silly('PuppetWeChatBridge', 'proxyWechaty(getMsgVideo, %d) exception: %s', id, e.message);
            throw e;
        }
    }
    async getMsgVoice(id) {
        config_1.log.verbose('PuppetWeChatBridge', 'getMsgVoice(%s)', id);
        try {
            return await this.proxyWechaty('getMsgVoice', id);
        }
        catch (e) {
            config_1.log.silly('PuppetWeChatBridge', 'proxyWechaty(getMsgVoice, %d) exception: %s', id, e.message);
            throw e;
        }
    }
    async getMsgPublicLinkImg(id) {
        config_1.log.verbose('PuppetWeChatBridge', 'getMsgPublicLinkImg(%s)', id);
        try {
            return await this.proxyWechaty('getMsgPublicLinkImg', id);
        }
        catch (e) {
            config_1.log.silly('PuppetWeChatBridge', 'proxyWechaty(getMsgPublicLinkImg, %d) exception: %s', id, e.message);
            throw e;
        }
    }
    async getMessage(id) {
        try {
            return await config_1.retry(async (retryException, attempt) => {
                config_1.log.silly('PuppetWeChatBridge', 'getMessage(%s) retry attempt %d', id, attempt);
                try {
                    ...
                    config_1.log.verbose('PuppetWeChatBridge', 'getMessage() proxyWechaty(getMessage, %s) exception: %s', id, e.message);
                    retryException(e);
                }
            });
        }
        catch (e) {
            config_1.log.error('PuppetWeChatBridge', 'promiseRetry() getContact() finally FAIL: %s', e.message);
            throw e;
        }
    }
    async getContact(id) {
        try {
            return await config_1.retry(async (retryException, attempt) => {
                config_1.log.silly('PuppetWeChatBridge', 'getContact(%s) retry attempt %d', id, attempt);
                try {
                    ...
                    config_1.log.verbose('PuppetWeChatBridge', 'getContact() proxyWechaty(getContact, %s) exception: %s', id, e.message);
                    retryException(e);
                }
            });
        }
    ...
    async getBaseRequest() {
        config_1.log.verbose('PuppetWeChatBridge', 'getBaseRequest()');
        try {
            return await this.proxyWechaty('getBaseRequest');
        }
        catch (e) {
            config_1.log.silly('PuppetWeChatBridge', 'proxyWechaty(getBaseRequest) exception: %s', e.message);
            throw e;
        }
    }
    async getPassticket() {
        config_1.log.verbose('PuppetWeChatBridge', 'getPassticket()');
        try {
            return await this.proxyWechaty('getPassticket');
        }
        catch (e) {
            config_1.log.silly('PuppetWeChatBridge', 'proxyWechaty(getPassticket) exception: %s', e.message);
            throw e;
        }
    }
    async getCheckUploadUrl() {
        config_1.log.verbose('PuppetWeChatBridge', 'getCheckUploadUrl()');
        try {
            return await this.proxyWechaty('getCheckUploadUrl');
        }
        catch (e) {
            config_1.log.silly('PuppetWeChatBridge', 'proxyWechaty(getCheckUploadUrl) exception: %s', e.message);
            throw e;
        }
    }
    async getUploadMediaUrl() {
        config_1.log.verbose('PuppetWeChatBridge', 'getUploadMediaUrl()');
        try {
            return await this.proxyWechaty('getUploadMediaUrl');
        }
        catch (e) {
            config_1.log.silly('PuppetWeChatBridge', 'proxyWechaty(getUploadMediaUrl) exception: %s', e.message);
            throw e;
        }
    }
    async sendMedia(mediaData) {
        config_1.log.verbose('PuppetWeChatBridge', 'sendMedia(mediaData)');
        ...
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'sendMedia() exception: %s', e.message);
            throw e;
        }
    }
    async forward(baseData, patchData) {
        config_1.log.verbose('PuppetWeChatBridge', 'forward()');
        ...
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'forward() exception: %s', e.message);
            throw e;
        }
    }
    /**
    ...
    async proxyWechaty(wechatyFunc, ...args) {
        config_1.log.silly('PuppetWeChatBridge', 'proxyWechaty(%s%s)', wechatyFunc, args.length === 0
            ? ''
            : ', ' + args.join(', '));
        if (!this.page) {
            throw new Error('no page');
        ...
        try {
            ...
            config_1.log.warn('PuppetWeChatBridge', 'proxyWechaty() noWechaty exception: %s', e);
            throw e;
        }
    ...
    `.replace(/[\n\s]+/, ' ');
        ...
        try {
            ...
            config_1.log.verbose('PuppetWeChatBridge', 'proxyWechaty(%s, %s) ', wechatyFunc, args.join(', '));
            config_1.log.warn('PuppetWeChatBridge', 'proxyWechaty() exception: %s', e.message);
            throw e;
        }
    }
    ding(data) {
        config_1.log.verbose('PuppetWeChatBridge', 'ding(%s)', data || '');
        this.proxyWechaty('ding', data)
            .then(dongData => {
            return this.emit('dong', dongData);
        })
            .catch(e => {
            config_1.log.error('PuppetWeChatBridge', 'ding(%s) exception: %s', data, e.message);
            this.emit('error', e);
        });
    }
    preHtmlToXml(text) {
        config_1.log.verbose('PuppetWeChatBridge', 'preHtmlToXml()');
        const preRegex = /^<pre[^>]*>([^<]+)<\/pre>$/i;
        const matches = text.match(preRegex);
        if (!matches) {
            return text;
        }
        return mod_1.unescapeHtml(matches[1]);
    }
    async innerHTML() {
    ...
    async testBlockedMessage(text) {
        ...
        config_1.log.verbose('PuppetWeChatBridge', 'testBlockedMessage(%s)', textSnip);
        let obj;
        try {
            ...
            config_1.log.warn('PuppetWeChatBridge', 'testBlockedMessage() toJson() exception: %s', e);
            return false;
        }
        if (!obj) {
            // FIXME: when will this happen?
            config_1.log.warn('PuppetWeChatBridge', 'testBlockedMessage() toJson(%s) return empty obj', textSnip);
            return false;
        }
        if (!obj.error) {
            return false;
        ...
        config_1.log.warn('PuppetWeChatBridge', 'testBlockedMessage() error.ret=%s', ret);
        if (ret === 1203) {
            // <error>
            // <ret>1203</ret>
    ...
    async clickSwitchAccount(page) {
        config_1.log.verbose('PuppetWeChatBridge', 'clickSwitchAccount()');
        ...
        try {
            ...
            if (button) {
                await button.click();
                config_1.log.silly('PuppetWeChatBridge', 'clickSwitchAccount() clicked!');
                return true;
            }
            else {
                config_1.log.silly('PuppetWeChatBridge', 'clickSwitchAccount() button not found');
                return false;
            }
        }
        catch (e) {
            config_1.log.silly('PuppetWeChatBridge', 'clickSwitchAccount() exception: %s', e);
            throw e;
        }
    }
    async hostname() {
        config_1.log.verbose('PuppetWeChatBridge', 'hostname()');
        if (!this.page) {
            throw new Error('no page');
        }
        try {
            const hostname = await this.page.evaluate(() => window.location.hostname);
            config_1.log.silly('PuppetWeChatBridge', 'hostname() got %s', hostname);
            return hostname;
        }
        catch (e) {
            config_1.log.error('PuppetWeChatBridge', 'hostname() exception: %s', e);
            this.emit('error', e);
            return null;
        }
    }
    async cookies(cookieList) {
        ...
        if (cookieList) {
            try {
                ...
                config_1.log.error('PuppetWeChatBridge', 'cookies(%s) reject: %s', cookieList, e);
                this.emit('error', e);
            }
            // RETURN
        }
     ...
     */
    entryUrl(cookieList) {
        config_1.log.verbose('PuppetWeChatBridge', 'cookieDomain(%s)', cookieList);
        /**
         * `?target=t` is from https://github.com/wechaty/wechaty-puppet-wechat/pull/129
         */
        const DEFAULT_URL = 'https://wx.qq.com?target=t';
        if (!cookieList || cookieList.length === 0) {
            config_1.log.silly('PuppetWeChatBridge', 'cookieDomain() no cookie, return default %s', DEFAULT_URL);
            return DEFAULT_URL;
        }
        const wxCookieList = cookieList.filter(c => /^webwx_auth_ticket|webwxuvid$/.test(c.name));
        if (!wxCookieList.length) {
            config_1.log.silly('PuppetWeChatBridge', 'cookieDomain() no valid cookie, return default hostname');
            return DEFAULT_URL;
        }
        let domain = wxCookieList[0].domain;
        if (!domain) {
            config_1.log.silly('PuppetWeChatBridge', 'cookieDomain() no valid domain in cookies, return default hostname');
            return DEFAULT_URL;
        }
        ...
        config_1.log.silly('PuppetWeChatBridge', 'cookieDomain() got %s', url);
        return url + '?target=t';
    }
    async reload() {
        config_1.log.verbose('PuppetWeChatBridge', 'reload()');
        if (!this.page) {
            throw new Error('no page');
        }
    ...
    async evaluate(fn, ...args) {
        config_1.log.silly('PuppetWeChatBridge', 'evaluate()');
        if (!this.page) {
            throw new Error('no page');
        }
        try {
            ...
            config_1.log.error('PuppetWeChatBridge', 'evaluate() exception: %s', e);
            this.emit('error', e);
            return null;
        }
    }
