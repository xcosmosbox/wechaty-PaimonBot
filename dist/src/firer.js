...
class Firer {
    ...
    async checkFriendConfirm(rawPayload) {
        ...
        config_1.log.silly('PuppetWeChatFirer', 'fireFriendConfirm(%s)', content);
    ...
    async checkRoomJoin(rawPayload) {
        ...
        try {
            ...
            config_1.log.silly('PuppetWeChatFirer', 'checkRoomJoin() "%s" is not a join message', text);
            return false; // not a room join message
        }
        config_1.log.silly('PuppetWeChatFirer', 'checkRoomJoin() inviteeList: %s, inviter: %s', inviteeNameList.join(','), inviterName);
        /**
        ...
        let ready = true;
        while (ttl-- > 0) {
            config_1.log.silly('PuppetWeChatFirer', 'fireRoomJoin() retry() ttl %d', ttl);
            if (!ready) {
            ...
            for (let i = 0; i < inviteeNameList.length; i++) {
                ...
                if (inviteeContactId) {
                    ...
                    try {
                        ...
                        config_1.log.warn('PuppetWeChatFirer', 'fireRoomJoin() contactPayload(%s) exception: %s', inviteeContactId, e.message);
                        ready = false;
                    }
                }
                else {
            ...
            if (ready) {
                config_1.log.silly('PuppetWeChatFirer', 'fireRoomJoin() resolve() inviteeContactIdList: %s, inviterContactId: %s', inviteeContactIdList.join(','), inviterContactId);
                /**
                 * Resolve All Payload again to make sure the data is ready.
                 */
                await Promise.all(inviteeContactIdList.map(id => this.puppet.contactPayload(id)));
                if (!inviterContactId) {
        ...
        config_1.log.warn('PuppetWeChatFier', 'fireRoomJoin() resolve payload fail.');
        return false;
    }
    /**
    ...
    async checkRoomLeave(rawPayload) {
        config_1.log.verbose('PuppetWeChatFirer', 'fireRoomLeave(%s)', rawPayload.Content);
        const roomId = rawPayload.FromUserName;
        let leaverName;
        let removerName;
        try {
            ...
            config_1.log.silly('PuppetWeChatFirer', 'fireRoomLeave() %s', e.message);
            return false;
        }
        config_1.log.silly('PuppetWeChatFirer', 'fireRoomLeave() got leaverName: %s', leaverName);
        /**
    ...
    async checkRoomTopic(rawPayload) {
        ...
        if (!changerContactId) {
            config_1.log.error('PuppetWeChatFirer', 'fireRoomTopic() changer contact not found for %s', changer);
            return false;
        }
        try {
            const timestamp = Math.floor(Date.now() / 1000); // in seconds
            ...
            config_1.log.error('PuppetWeChatFirer', 'fireRoomTopic() co exception: %s', e.stack);
            return false;
        }
    }
    /**
    ...
    parseRoomJoin(content) {
        config_1.log.verbose('PuppetWeChatFirer', 'parseRoomJoin(%s)', content);
